...
const useDraggableAndResizable = (maximized = false): DraggableAndResizable => {
  ...
  const updateSize = useCallback<RndResizeCallback>(
    (_event, _direction, { style: { height: elementHeight, width: elementWidth } }, _delta, { x: positionX, y: positionY }) => {
  ...
  const taskbarHeight = useMemo(() => Number(sizes.taskbar.height.replace("px", "")), [sizes.taskbar.height]);
