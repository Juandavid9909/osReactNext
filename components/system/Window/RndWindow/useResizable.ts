...
const useResizable = (maximized = false, autoSizing = false, size = DEFAULT_WINDOW_SIZE): Resizable => {
  ...
  return [
    {
      height: maximized ? `${ window.innerHeight - Number(stripUnit(taskbar.height)) }px` : height,
